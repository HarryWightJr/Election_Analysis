...
with open(file_to_load) as election_data:
    ...
    for row in reader:
        ...
        if counties not in counties_listed:

            # 4b: Add the existing county to the list of counties.
            counties_listed.append(counties)
...
with open(file_to_save, "w") as txt_file:
    ...
    for counties in county_votes:
        ...
        county_results = (f"{counties}: {votes_percentage:.1f}% ({votes:,})\n")
        print(county_results)
         # 6e: Save the county votes to a text file.
        txt_file.write(county_results)
         # 6f: Write an if statement to determine the winning county and get its vote count.
        if votes > votes_in_largest_county:
            largest_turnout_county = counties
            votes_in_largest_county = votes
